...
<p>In the output:</p>
<ul>
<li><code class="language-text">wanted</code> is the maximum version of the package that satisfies the semver
range specified in <code class="language-text">package.json</code>. If there's no available semver range (i.e.
you're running <code class="language-text">npm outdated --global</code>, or the package isn't included in
<code class="language-text">package.json</code>), then <code class="language-text">wanted</code> shows the currently-installed version.</li>
<li><code class="language-text">latest</code> is the version of the package tagged as latest in the registry.
Running <code class="language-text">npm publish</code> with no special configuration will publish the package
...
<div class="gatsby-highlight" data-language="json"><pre class="language-json"><code class="language-json"><span class="token punctuation">{</span>
  ...
  <span class="token property">"nothingness"</span><span class="token operator">:</span> <span class="token string">"github:othiym23/nothingness#master"</span><span class="token punctuation">,</span>
  <span class="token property">"npm"</span><span class="token operator">:</span> <span class="token string">"^3.5.1"</span><span class="token punctuation">,</span>
  <span class="token property">"once"</span><span class="token operator">:</span> <span class="token string">"^1.3.1"</span>
<span class="token punctuation">}</span></code></pre></div>
<p>A few things to note:</p>
<ul>
<li><code class="language-text">glob</code> requires <code class="language-text">^5</code>, which prevents npm from installing <code class="language-text">glob@6</code>, which is
outside the semver range.</li>
<li>Git dependencies will always be reinstalled, because of how they're specified.
The installed committish might satisfy the dependency specifier (if it's
something immutable, like a commit SHA), or it might not, so <code class="language-text">npm outdated</code> and
<code class="language-text">npm update</code> have to fetch Git repos to check. This is why currently doing a
reinstall of a Git dependency always forces a new clone and install.</li>
<li><code class="language-text">npm@3.5.2</code> is marked as "wanted", but "latest" is <code class="language-text">npm@3.5.1</code> because npm
uses dist-tags to manage its <code class="language-text">latest</code> and <code class="language-text">next</code> release channels. <code class="language-text">npm update</code>
will install the <em>newest</em> version, but <code class="language-text">npm install npm</code> (with no semver range)
will install whatever's tagged as <code class="language-text">latest</code>.</li>
<li><code class="language-text">once</code> is just plain out of date. Reinstalling <code class="language-text">node_modules</code> from scratch or
running <code class="language-text">npm update</code> will bring it up to spec.</li>
</ul>
