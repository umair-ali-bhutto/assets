...
<span class="token function">npm</span> cache <span class="token function">add</span> <span class="token operator">&lt;</span>folder<span class="token operator">></span>
<span class="token function">npm</span> cache <span class="token function">add</span> <span class="token operator">&lt;</span>tarball url<span class="token operator">></span>
<span class="token function">npm</span> cache <span class="token function">add</span> <span class="token operator">&lt;</span>name<span class="token operator">></span>@<span class="token operator">&lt;</span>version<span class="token operator">></span>

<span class="token function">npm</span> cache clean <span class="token punctuation">[</span><span class="token operator">&lt;</span>path<span class="token operator">></span><span class="token punctuation">]</span>
aliases: <span class="token function">npm</span> cache clear, <span class="token function">npm</span> cache <span class="token function">rm</span>

<span class="token function">npm</span> cache verify</code></pre></div>
...
add data to the local installation cache explicitly.</li>
<li>clean:
Delete all data out of the cache folder.</li>
<li>verify:
Verify the contents of the cache folder, garbage collecting any unneeded data,
and verifying the integrity of the cache index and all cached data.</li>
</ul>
...
<p>npm stores cache data in an opaque directory within the configured <code class="language-text">cache</code>,
named <code class="language-text">_cacache</code>. This directory is a <code class="language-text">cacache</code>-based content-addressable cache
that stores all http request data as well as other package-related data. This
directory is primarily accessed through <code class="language-text">pacote</code>, the library responsible for
...
signal to <code class="language-text">pacote</code> that the data must be refetched, which it will do
automatically. For this reason, it should never be necessary to clear the cache
for any reason other than reclaiming disk space, thus why <code class="language-text">clean</code> now requires
<code class="language-text">--force</code> to run.</p>
<p>There is currently no method exposed through npm to inspect or directly manage
the contents of this cache. In order to access it, <code class="language-text">cacache</code> must be used
directly.</p>
<p>npm will not remove data by itself: the cache will grow as new packages are
installed.</p>
<h3 id="a-note-about-the-caches-design" style="position:relative;"><a href="#a-note-about-the-caches-design" aria-label="a note about the caches design permalink" class="header-link-class before"><svg aria-hidden="true" height="20" version="1.1" viewBox="0 0 16 16" width="20"><path fill-rule="evenodd" d="M4 9h1v1H4c-1.5 0-3-1.69-3-3.5S2.55 3 4 3h4c1.45 0 3 1.69 3 3.5 0 1.41-.91 2.72-2 3.25V8.59c.58-.45 1-1.27 1-2.09C10 5.22 8.98 4 8 4H4c-.98 0-2 1.22-2 2.5S3 9 4 9zm9-3h-1v1h1c1 0 2 1.22 2 2.5S13.98 12 13 12H9c-.98 0-2-1.22-2-2.5 0-.83.42-1.64 1-2.09V6.25c-1.09.53-2 1.84-2 3.25C6 11.31 7.55 13 9 13h4c1.45 0 3-1.69 3-3.5S14.5 6 13 6z"></path></svg></a>A note about the cache's design</h3>
...
<p>To run an offline verification of existing cache contents, use <code class="language-text">npm cache
verify</code>.</p>
...
<p>Default: <code class="language-text">~/.npm</code> on Posix, or <code class="language-text">%AppData%/npm-cache</code> on Windows.</p>
