...
<div class="gatsby-highlight" data-language="bash"><pre class="language-bash"><code class="language-bash">$ <span class="token builtin class-name">cd</span> ./my/npm/project
$ <span class="token function">npm</span> <span class="token function">install</span>
added <span class="token number">154</span> packages <span class="token keyword">in</span> 10s
$ <span class="token function">ls</span> <span class="token operator">|</span> <span class="token function">grep</span> package-lock</code></pre></div>
<p>Run <code class="language-text">npm ci</code> in that project</p>
<div class="gatsby-highlight" data-language="bash"><pre class="language-bash"><code class="language-bash">$ <span class="token function">npm</span> ci
added <span class="token number">154</span> packages <span class="token keyword">in</span> 5s</code></pre></div>
<p>Configure Travis to build using <code class="language-text">npm ci</code> instead of <code class="language-text">npm install</code>:</p>
<div class="gatsby-highlight" data-language="bash"><pre class="language-bash"><code class="language-bash"><span class="token comment"># .travis.yml</span>
install:
- <span class="token function">npm</span> ci
<span class="token comment"># keep the npm cache around to speed up installs</span>
cache:
  directories:
...
<p>In short, the main differences between using <code class="language-text">npm install</code> and <code class="language-text">npm ci</code> are:</p>
<ul>
<li>The project <strong>must</strong> have an existing <code class="language-text">package-lock.json</code> or <code class="language-text">npm-shrinkwrap.json</code>.</li>
<li>If dependencies in the package lock do not match those in <code class="language-text">package.json</code>, <code class="language-text">npm ci</code> will exit with an error, instead of updating the package lock.</li>
<li><code class="language-text">npm ci</code> can only install entire projects at a time: individual dependencies cannot be added with this command.</li>
<li>If a <code class="language-text">node_modules</code> is already present, it will be automatically removed before <code class="language-text">npm ci</code> begins its install.</li>
<li>It will never write to <code class="language-text">package.json</code> or any of the package-locks: installs are essentially frozen.</li>
</ul>
